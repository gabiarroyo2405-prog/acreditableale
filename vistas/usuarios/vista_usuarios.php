<?php 
/**
 * Archivo: vistas/usuarios/vista_usuarios.php
 * Objetivo: Interfaz de usuario para la gestion de usuarios con datos reales y operaciones CRUD (Hard Delete)
 */

// 1. DISEÑO: Cargamos los fragmentos de la plantilla (Layouts)
require_once '../../vistas/plantillas/cabecera.php'; 

// 2. LOGICA: Importamos y usamos el controlador de usuarios
use Controladores\UsuarioControlador;

// Instanciamos el controlador para poder pedirle los datos
...
<div id="content-wrapper" class="d-flex flex-column">
    <div id="content">
        ...
        <div class="container-fluid">
            ...
            <!-- BLOQUE DE NOTIFICACIONES: Muestra mensajes de exito o error -->
            ...
            <?php elseif (isset($_GET['eliminado'])): ?>
                <div class="alert alert-warning alert-dismissible fade show" role="alert">
                    🗑️ Usuario eliminado permanentemente de la base de datos.
                    <button type="button" class="close" data-dismiss="alert" aria-label="Cerrar">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
            ...
            <!-- TABLA DE DATOS: Muestra la informacion de la base de datos -->
                                    ...
                                    <?php foreach ($listaUsuarios as $usuario): ?>
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                <!-- Boton para Eliminar Físicamente -->
                                                <button class="btn btn-danger btn-sm btn-eliminar" title="Eliminar permanentemente"
                                                        data-id="<?php echo $usuario['idUsuario']; ?>"
                                                        data-nombre="<?php echo htmlspecialchars($usuario['nombreCompleto']); ?>"
                                                        data-toggle="modal"
                                                        data-target="#modalConfirmarEliminar">
        ...
        <!-- MODAL PARA NUEVO USUARIO (Operacion: CREACION) -->
        <div class="modal fade" id="modalNuevoUsuario" tabindex="-1" role="dialog" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <!-- FORMULARIO: Envia los datos al controlador de PHP -->
                    <form action="../../controladores/UsuarioControlador.php" method="POST">
                        <input type="hidden" name="accion" value="guardar">
                        <div class="modal-body">
                            <div class="form-group">
                                <label>Nombre y Apellido</label>
                                <input type="text" name="nombreCompleto" class="form-control" placeholder="Ej. Juan Perez" required>
                            </div>
                            <div class="form-group row">
                                <div class="col-sm-6 mb-3 mb-sm-0">
                                    <label>Usuario (Login)</label>
                                    <input type="text" name="usuarioLogin" class="form-control" placeholder="juan.perez" required>
                                </div>
                                <div class="col-sm-6">
                                    <label>Contrasena</label>
                                    <input type="password" name="passwordHash" class="form-control" placeholder="****" required>
                                </div>
                            </div>
                            <div class="form-group">
        ...
        <!-- MODAL PARA EDITAR USUARIO (Operacion: UPDATE) -->
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <form action="../../controladores/UsuarioControlador.php" method="POST">
                        ...
                        <!-- ID oculto importante para que el backend sepa de quién trata el Update -->
                        <input type="hidden" name="idUsuario" id="editIdUsuario">
                        
                        <div class="modal-body">
                            <div class="form-group">
                                <label>Nombre y Apellido</label>
                                <input type="text" name="nombreCompleto" id="editNombre" class="form-control" required>
                            </div>
                            <div class="form-group row">
                                <div class="col-sm-6 mb-3 mb-sm-0">
                                    <label>Usuario (Login)</label>
                                    <input type="text" name="usuarioLogin" id="editLogin" class="form-control" required>
                                </div>
                                <div class="col-sm-6">
                                    <label>Rol en la Parroquia</label>
                        ...
                        <div class="modal-footer">
                            <button class="btn btn-secondary" type="button" data-dismiss="modal">Cancelar</button>
                            <button class="btn btn-warning" type="submit"><i class="fas fa-save mr-1"></i> Guardar Cambios</button>
                        </div>
                    </form>
                </div>
            </div>
        ...
        <!-- MODAL CONFIRMAR ELIMINACIÓN PERMANENTE (Operacion: HARD DELETE) -->
        <div class="modal fade" id="modalConfirmarEliminar" tabindex="-1" role="dialog" aria-hidden="true">
            <div class="modal-dialog modal-sm" role="document">
                <div class="modal-content">
                    <div class="modal-header bg-danger text-white">
                        <h5 class="modal-title"><i class="fas fa-exclamation-triangle mr-2"></i>Confirmar Borrado</h5>
                    ...
                    <div class="modal-body text-center">
                        <p class="mb-1">Estás a punto de eliminar físicamente al usuario:</p>
                        <strong id="nombreAEliminar" class="text-danger d-block my-2 h5"></strong>
                        <small class="text-muted">⚠️ Esta acción no se puede deshacer.</small>
                    </div>

                    <form action="../../controladores/UsuarioControlador.php" method="POST">
                        <input type="hidden" name="accion" value="eliminar">
                        <input type="hidden" name="idUsuario" id="eliminarIdUsuario">

                        <div class="modal-footer justify-content-center">
                            <button class="btn btn-secondary" type="button" data-dismiss="modal">Cancelar</button>
                            <button class="btn btn-danger" type="submit"><i class="fas fa-trash mr-1"></i> Sí, Eliminar</button>
                        </div>
                    </form>
                </div>
            </div>
...
<!-- SCRIPT PUNTUAL PARA MODALES INTERACTIVOS -->
<script>
    document.addEventListener("DOMContentLoaded", function() {
        // Logica para rellenar el modal de edición
        ...
        // Logica para rellenar modal de eliminación
        document.querySelectorAll('.btn-eliminar').forEach(function(boton) {
            boton.addEventListener('click', function() {
                document.getElementById('eliminarIdUsuario').value = this.getAttribute('data-id');
                document.getElementById('nombreAEliminar').textContent = this.getAttribute('data-nombre');
            });
        });
    });
</script>
...
<?php 
// 4. CIERRE: Cargamos la parte final del diseño y los scripts de JavaScript
