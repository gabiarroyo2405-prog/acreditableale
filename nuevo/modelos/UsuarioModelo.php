...
class UsuarioModelo {
    ...
    public function __construct() {
        // Al instanciar el modelo, obtenemos la conexión única a la base de datos
    ...
    public function insertar($nombreCompleto, $rol, $usuarioLogin, $contrasena) {
        try {
            ...
            $exito = $consulta->execute([
                ...
                ':rol' => $rol,
                ':login' => $usuarioLogin,
                ':password' => $contrasena // Nota: En la Clase 4 enseñaremos a encriptar contraseñas
            ]);
    ...
    // MÉTODO: Eliminar usuario físicamente (Operación: HARD DELETE)
    public function eliminar($idUsuario) {
        try {
            // ⚠️ PELIGRO: DELETE FÍSICO. ELIMINA EL REGISTRO DE MARIA_DB PARA SIEMPRE.
            $consulta = $this->db->prepare("
            ...
            // count de filas afectadas nos confirmará si el borrado tuvo éxito.
            return $consulta->rowCount() > 0;
            
        } catch (\PDOException $error) {
            error_log("Error al eliminar usuario: " . $error->getMessage());
            return false;
        }
    }
}
